...
def gerar_boxplot_geral(data_name, metrica):
    ...
    alg1 = np.asarray(pd.read_excel(io='arquivos_lista03/'+data_name+'.xls', sheet_name='OLA')[metrica])
    data.append(alg1)
    alg2 = np.asarray(pd.read_excel(io='arquivos_lista03/'+data_name+'.xls', sheet_name='LCA')[metrica])
    data.append(alg2)
    alg3 = np.asarray(pd.read_excel(io='arquivos_lista03/'+data_name+'.xls', sheet_name='KNORA-E')[metrica])
    data.append(alg3)
    alg4 = np.asarray(pd.read_excel(io='arquivos_lista03/'+data_name+'.xls', sheet_name='KNORA-U')[metrica])
    data.append(alg4)
    alg5 = np.asarray(pd.read_excel(io='arquivos_lista03/'+data_name+'.xls', sheet_name='Arquitetura')[metrica])
    data.append(alg5)
