...
def gerar_boxplot_validacao(data_name, metrica):
    ...
    for i in validacao:
        ...
        alg1 = np.asarray(pd.read_excel(io='arquivos_lista02/'+data_name+'-val-'+i+'.xls', sheetname='Bagging+REP')[metrica])
        alg2 = np.asarray(pd.read_excel(io='arquivos_lista02/'+data_name+'-val-'+i+'.xls', sheetname='Bagging+OGOB')[metrica])
...
def gerar_boxplot_metrica_geral_twodata(metrica):
    ...
    for i in validacao:
        ...
        alg1 = np.asarray(pd.read_excel(io='arquivos_lista02/kc1-val-'+i+'.xls', sheetname='Bagging+REP')[metrica])
        alg2 = np.asarray(pd.read_excel(io='arquivos_lista02/kc1-val-'+i+'.xls', sheetname='Bagging+OGOB')[metrica])
        
        alg5 = np.asarray(pd.read_excel(io='arquivos_lista02/kc2-val-'+i+'.xls', sheetname='Bagging+REP')[metrica])
        alg6 = np.asarray(pd.read_excel(io='arquivos_lista02/kc2-val-'+i+'.xls', sheetname='Bagging+OGOB')[metrica])
...
#gerar_boxplot_metrica_geral_twodata(metricas[6])
